...
const getAllUsers = async (req, res) => {
    try {
        const [result] = await db.execute('SELECT id, name, education, created_at FROM users');
        res.json(result);
        } catch (error) {
...
const getUserBy = async (req, res) => {
    try {
        ...
        const allowedParams = ['id', 'name', 'education']; // Define allowed columns
        if (!allowedParams.includes(param)) {
            return res.status(400).json({ error: "Invalid search parameter" });
        }

        // Query database dynamically
        const query = `SELECT id, name, education, created_at FROM users WHERE ${param} = ?`;
...
const deleteUser = async (req, res) => {
    try {
        ...
        const [oldUser] = await db.execute('SELECT id, name, education FROM users WHERE id = ?', [id]);
...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        const { username, education, password } = req.body;
        ...
        const [result] = await db.execute(
            ...
            [username, education, hashedPassword]
        );
